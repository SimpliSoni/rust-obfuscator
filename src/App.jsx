...
const createIcon = (svgContent) => memo(({ className }) => (
  <svg
...
const SlidersHorizontal = createIcon('<line x1="21" x2="14" y1="4" y2="4" /><line x1="10" x2="3" y1="4" y2="4" /><line x1="21" x2="12" y1="12" y2="12" /><line x1="8" x2="3" y1="12" y2="12" /><line x1="21" x2="16" y1="20" y2="20" /><line x1="12" x2="3" y1="20" y2="20" /><line x1="14" x2="14" y1="2" y2="6" /><line x1="8" x2="8" y1="10" y2="14" /><line x1="16" x2="16" y1="18" y2="22" />');
const Zap = createIcon('<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2" />');
const ZapOff = createIcon('<polyline points="12.41 6.75 13 2 10.57 4.92" /><polyline points="18.57 12.91 21 10 15.66 10" /><polyline points="8 8 3 14 12 14 11 22 16 16" /><line x1="1" x2="23" y1="1" y2="23" />');
...
const useObfuscationCore = () => {
    ...
    const obfuscateNumbers = useMemo(() => (code) => code.replace(/\b(\d+)\b/g, (match, numStr) => {
    ...
    const encryptString = useMemo(() => (code) => code.replace(/"([^"]+)"/g, (match, str) => {
    ...
    const splitString = useMemo(() => (code) => code.replace(/"([^"]+)"/g, (match, str) => {
    ...
    const addDeadCode = useMemo(() => (code) => {
        const snippets = [' if false { let _unused = 42; }', ' let _dummy = 0; let _dummy2 = _dummy * 0;'];
        return code.replace(/(fn\s+\w+[^{]*\{)/g, (match) => `${match}\n    ${snippets[Math.floor(Math.random() * snippets.length)]}`);
    }, []);
    
    const obfuscateVariableNames = useMemo(() => (code) => {
        ...
        variables.forEach((newName, oldName) => { result = result.replace(new RegExp(`\\b${oldName}\\b`, 'g'), newName); });
        return result;
    }, []);

    const obfuscateControlFlow = useMemo(() => (code) => code.replace(/if\s+([^{]+)\s*\{([^}]+)\}\s*else\s*\{([^}]+)\}/g, (match, condition, ifBlock, elseBlock) => `match ${condition} { true => {${ifBlock}}, false => {${elseBlock}} }`), []);
    
    const createStringConstants = useMemo(() => (code) => {
        const strings = [];
        let counter = 0;
    ...
    const deobfuscateNumbers = useMemo(() => (code) => code.replace(/\((\s*\d+\s*[\+\-\*]\s*\d+\s*)\)/g, (match, expr) => {
        try { return new Function(`return ${expr}`)(); } catch { return match; }
    }), []);
    
    const simplifyPredicates = useMemo(() => (code) => code.replace(/if\s+\([^)]+\)/g, (match) => {
        if (match.includes('==') || match.includes('>') || match.includes('||') || match.includes('!=')) return 'if true';
        return match;
    }), []);
    
    const removeDeadCode = useMemo(() => (code) => code.replace(/\s*if false \{[^}]+\}/g, '').replace(/\s*let _dummy = 0; let _dummy2 = _dummy \* 0;/g, ''), []);
    
    const simplifyControlFlow = useMemo(() => (code) => code.replace(/match\s+([^\{]+)\s*\{\s*true\s*=>\s*\{([^}]+)\},\s*false\s*=>\s*\{([^}]+)\}\s*\}/g, (match, condition, trueBlock, falseBlock) => `if ${condition} {\n${trueBlock}\n} else {\n${falseBlock}\n}`), []);
    
    const inlineStringConstants = useMemo(() => (code) => {
    ...
    return { 
        obfuscateNumbers, addOpaquePredicates, encryptString, splitString, addDeadCode,
        obfuscateVariableNames, obfuscateControlFlow, createStringConstants,
        deobfuscateNumbers, simplifyPredicates, removeDeadCode, simplifyControlFlow,
        ...
        <nav className="flex justify-center mb-8 animate-fade-in-down" style={{ animationDelay: '100ms' }}>
            <div className="flex flex-wrap justify-center p-1 bg-black/30 backdrop-blur-sm border border-blue-500/20 rounded-xl">
                {navItems.map(item => (
                    ...
                    >
                        {activeView === item && <span className="absolute inset-0 bg-blue-500/30 rounded-lg -z-10 motion-safe:animate-nav-pulse" />}
                        {item}
                    </button>
                ))}
            </div>
...
const ObfuscatorView = ({ core, stats, setStats, setCodeAnalysis }) => {
    const [inputCode, setInputCode] = useState(`fn main() {\n    let x = 42;\n    let name = "Rust";\n    if true {\n        println!("Hello, {}!", name);\n        println!("The answer is: {}", x);\n    }\n}`);
    const [outputCode, setOutputCode] = useState('');
    const [copied, setCopied] = useState(false);
    const [options, setOptions] = useState({
        numbers: true, predicates: true, deadCode: true,
        variables: true, controlFlow: false, stringConstants: false,
    });
    ...
    const runObfuscation = () => {
        ...
        setCodeAnalysis({ functions: funcs, variables: vars, loops, conditionals: conds, strings: Math.floor(strs), comments: 0 });
        setStats(prev => ({
    ...
    return (
        <div className="animate-fade-in">
                    ...
                    {Object.entries(options).map(([key, value]) => (
                        <label key={key} className="flex items-center gap-3 cursor-pointer group">
                            <div className="relative">
                                <input type="checkbox" checked={value} onChange={(e) => setOptions(prev => ({ ...prev, [key]: e.target.checked }))}
                                ...
                                />
                                <Check className={`w-4 h-4 absolute top-0.5 left-0.5 text-black transition-opacity duration-200 ${value ? 'opacity-100' : 'opacity-0'}`} />
                            </div>
                            <span className="capitalize text-gray-300 group-hover:text-blue-300 transition">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                        </label>
                    ))}
                </div>
            </div>
            <div className="grid md:grid-cols-2 gap-6">
                <div>
                    <label htmlFor="source-code-input" className="text-lg font-semibold mb-2 text-gray-300 block">Source Code</label>
                    <div className="relative">
                        <textarea id="source-code-input" value={inputCode} onChange={(e) => setInputCode(e.target.value)} className="w-full h-96 p-4 bg-black/50 border border-blue-500/20 rounded-lg font-mono text-sm text-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition" placeholder="Paste your Rust code here..." />
                    </div>
                </div>
                <div>
                    <div className="flex items-center justify-between mb-2">
                        ...
                        {outputCode && (
                            <button onClick={copyToClipboard} className="flex items-center gap-2 px-3 py-1 bg-gray-700 hover:bg-blue-500/20 text-gray-300 hover:text-blue-300 border border-gray-600 hover:border-blue-500/30 rounded-lg transition">
                                {copied ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4" />}
                                {copied ? 'Copied' : 'Copy'}
                            </button>
                        )}
                    </div>
                    <textarea value={outputCode} readOnly aria-label="Obfuscated Code Output" className="w-full h-96 p-4 bg-black/50 border border-blue-500/20 rounded-lg font-mono text-sm text-gray-200" placeholder="Transformed code will appear here..." />
                </div>
            </div>
            <div className="mt-6 flex justify-center items-center gap-4">
                <button onClick={runObfuscation} className="group relative px-8 py-3 bg-gradient-to-r from-blue-500 to-cyan-500 hover:shadow-lg hover:shadow-cyan-500/30 rounded-lg font-semibold text-white transform transition hover:scale-105 flex items-center gap-2">
                    <Zap className="w-5 h-5" />
                    Obfuscate
                </button>
                <button onClick={runDeobfuscation} className="group relative px-8 py-3 bg-gray-700 hover:bg-gray-600 rounded-lg font-semibold text-white transform transition hover:scale-105 flex items-center gap-2">
...
const VisualizationView = ({ codeAnalysis, stats }) => {
    ...
    useEffect(() => {
        ...
        const nucleusMat = new THREE.MeshStandardMaterial({ color: 0x00ffff, wireframe: true, emissive: 0x00ffff, emissiveIntensity: 0.5 });
        ...
        <div className="animate-fade-in">
            ...
            <div className="w-full h-[65vh] max-h-[700px] rounded-xl border border-blue-500/20 overflow-hidden bg-black/50 backdrop-blur-sm shadow-2xl shadow-blue-500/10 relative">
                ...
                <div className="absolute top-4 left-4 text-white bg-black/50 p-3 rounded-lg border border-gray-700 text-xs">
                    <p className="font-semibold text-blue-300">Functions <span className="text-purple-400 ml-2 font-mono">{codeAnalysis.functions}</span></p>
                    <p className="font-semibold text-blue-300">Variables <span className="text-green-400 ml-2 font-mono">{codeAnalysis.variables}</span></p>
                    <p className="font-semibold text-blue-300">Loops <span className="text-yellow-400 ml-2 font-mono">{codeAnalysis.loops}</span></p>
                    <p className="font-semibold text-blue-300">Conditionals <span className="text-orange-400 ml-2 font-mono">{codeAnalysis.conditionals}</span></p>
                </div>
            </div>
        </div>
    );
        ...
        return (
            <div className="relative flex flex-col items-center justify-center">
                <svg width="120" height="120" viewBox="0 0 120 120" className="transform -rotate-90">
                    ...
                    <circle cx="60" cy="60" r={radius} strokeWidth="10" className={`stroke-current ${color}`} fill="transparent" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" style={{ transition: 'stroke-dashoffset 0.5s ease-out' }} />
                </svg>
    ...
    const MetricCard = ({ title, value, icon: Icon, color, suffix = '' }) => {
        ...
        return (
            <div className="bg-black/40 p-4 rounded-xl border border-blue-500/20 backdrop-blur-sm flex items-center gap-4 transform transition hover:scale-105 hover:border-blue-500/50">
                <div className={`p-3 rounded-lg bg-gray-800/50 ${colorClasses[color]}`}><Icon className="w-6 h-6" /></div>
                <div>
                    <div className="text-gray-400 text-sm">{title}</div>
                    <div className={`text-2xl font-bold ${colorClasses[color]}`}>{value}{suffix}</div>
                </div>
            </div>
        );
    };
...
const LibraryView = ({ core }) => {
    ...
    const techniques = useMemo(() => ({
        arithmetic: {
            ...
            description: 'This technique replaces simple numeric literals with more complex mathematical expressions that evaluate to the original number. It makes the code harder to read at a glance by obscuring the actual values being used.',
        ...
        predicates: {
            ...
            description: 'Opaque predicates involve inserting conditional branches (like if statements) that always evaluate to the same result. This adds unnecessary complexity and "dead code" paths to confuse reverse engineers.',
            pros: ['Breaks linear code flow, confusing static analysis tools.', 'Can be used to insert large amounts of dead code.'],
            cons: ['Modern compilers may optimize away simple predicates.', 'Can be fingerprinted if the same patterns are used repeatedly.'],
            demoFunc: core.addOpaquePredicates,
            exampleCode: 'if true {\n    println!("Critical operation");\n}'
        },
        encryption: {
            title: 'String Encryption',
            description: 'This method hides string literals by encrypting them at compile-time and decrypting them only when needed at runtime. This prevents an attacker from easily finding sensitive information like API keys or error messages in the compiled binary.',
            pros: ['Effectively hides sensitive string data.', 'Makes it much harder to understand the program\'s purpose.'],
            cons: ['Adds runtime performance overhead due to decryption.', 'Requires a decryption routine to be included in the binary.'],
            demoFunc: core.encryptString,
            exampleCode: 'let api_key = "SECRET_KEY";\nlet message = "Hello World";'
        },
        splitting: {
            title: 'String Splitting',
            description: 'A simpler alternative to encryption, this technique breaks a single string literal into multiple smaller parts that are concatenated at runtime. This can prevent simple text searches for the full string within the compiled binary.',
        ...
        return (
            <div className="bg-black/30 p-6 rounded-lg border border-blue-500/20 animate-fade-in">
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <h4 className="font-semibold text-green-400 mb-2 flex items-center gap-2"><PlusCircle className="w-5 h-5" /> Pros</h4>
                        <ul className="list-disc list-inside text-gray-300 space-y-1 text-sm">{pros.map(p => <li key={p}>{p}</li>)}</ul>
                    </div>
                    <div>
                        <h4 className="font-semibold text-red-400 mb-2 flex items-center gap-2"><MinusCircle className="w-5 h-5" /> Cons</h4>
                        <ul className="list-disc list-inside text-gray-300 space-y-1 text-sm">{cons.map(c => <li key={c}>{c}</li>)}</ul>
                    </div>
                </div>
                <div className="bg-black/40 p-4 rounded-lg border border-gray-600">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                        <textarea value={input} onChange={(e) => setInput(e.target.value)} className="w-full h-48 p-2 bg-gray-900 border border-gray-700 rounded font-mono text-xs focus:ring-1 focus:ring-blue-500 outline-none" />
                        <textarea value={output} readOnly className="w-full h-48 p-2 bg-gray-900 border border-gray-700 rounded font-mono text-xs" placeholder="Result..." />
                    </div>
                    <button onClick={handleApply} className="mt-3 px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded text-white font-semibold text-sm transition">Apply</button>
                </div>
            </div>
        );
    };
            ...
            <div className="flex justify-center mb-6">
                <div className="flex flex-wrap justify-center p-1 bg-gray-800/80 backdrop-blur-sm border border-blue-500/20 rounded-lg">
                    {Object.keys(techniques).map(key => (
                        <button key={key} onClick={() => setActiveTab(key)} className={`px-4 py-2 rounded-md transition text-sm sm:text-base capitalize ${activeTab === key ? 'bg-blue-500 text-white shadow-lg shadow-blue-500/30' : 'hover:bg-gray-700'}`}>{key}</button>
                    ))}
                </div>
            </div>
...
const RustObfuscator = () => {
    ...
    const [stats, setStats] = useState({
        totalObfuscations: 0, linesProcessed: 0, securityLevel: 0,
        codeComplexity: 0, obfuscationLevel: 0, performanceImpact: 0,
    });
    const [codeAnalysis, setCodeAnalysis] = useState({
        functions: 1, variables: 2, loops: 0,
        conditionals: 1, strings: 2, comments: 0,
    });
    ...
    return (
        <div className="min-h-screen bg-gray-900 text-gray-100 p-4 sm:p-6 font-sans relative overflow-hidden">
            ...
            `}</style>
            <div className="absolute inset-0 z-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle at 25% 25%, var(--glow-color), transparent 30%), radial-gradient(circle at 75% 75%, var(--glow-color), transparent 30%)' }} />
            ...
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <main>
                    {activeView === 'obfuscator' && <ObfuscatorView core={core} stats={stats} setStats={setStats} setCodeAnalysis={setCodeAnalysis} />}
